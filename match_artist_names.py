...
def levenshtein(s1, s2):
    ...
    for (i, c1) in enumerate(s1):
        ...
        for (j, c2) in enumerate(s2):
            # j + 1 instead of j because previous_row and current_row are one
...
def main():
    artist_to_movement = pickle.load(open("artist_to_movement.pydict", "rb"))
    ...
    for (i, artist) in enumerate(painting_artists):
        ...
        while try_again:
            ...
            try:
                if artist not in potential_matches:
                    selection = input("Pick one or none: ")
                    if selection == "":
                        raise SyntaxError
                    else:
                ...
                if 0 <= selection < len(closest_matches):
                    ...
                    pickle.dump(artist_match, open("artist_match.pydict", "wb"))
                    try_again = False
                else:
                    raise ValueError
            except SyntaxError:

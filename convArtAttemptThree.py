...
train_set = cPickle.load(open("train_set.numpy"))
valid_set = cPickle.load(open("valid_set.numpy"))
test_set = cPickle.load(open("test_set.numpy"))
...
layer0_filter_height = 11
layer0_filter_width = 11

# Construct the first convolutional pooling layer:
# filtering reduces the image size to (100 - 11 + 1 , 100 - 11 + 1) = (90, 90)
# Why + 1? Draw a picture. It makes sense.
# maxpooling reduces this further to (90 / 2, 90 / 2) = (45, 45)
# 4D output tensor is thus of shape (batch_size, nkerns[0], 45, 45)
...
layer1_filter_height = 10
layer1_filter_width = 10

# Construct the second convolutional pooling layer
# filtering reduces the image size to (45 - 10 + 1, 45 - 10 + 1) = (36, 36)
# maxpooling reduces this further to (36 / 2, 36 / 2) = (18, 18)
# 4D output tensor is thus of shape (nkerns[0], nkerns[1], 18, 18)
...
# This will generate a matrix of shape (batch_size, nkerns[2] * 7 * 7),
# or (500, 50 * 9 * 9) = (500, 800) with the default values.
...
while (epoch < n_epochs) and (not done_looping):
    ...
    for minibatch_index in xrange(n_train_batches):
        ...
        if iter % 3 == 0:
            end_time = time.clock()
            print >> sys.stderr, ('The code has been running for %.2fm' % ((end_time - start_time) / 60.))
...
baseline = float(counts[0][1]) / float(len(testLabels))

weights = layer0.W.get_value(borrow=True).T
biases = layer0.b.get_value(borrow=True).T
cPickle.dump(weights, open("weights.numpy", "w"))
cPickle.dump(biases, open("biases.numpy", "w"))